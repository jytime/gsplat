...
class DefaultStrategy(Strategy):
    ...
    def step_post_backward(
        ...
        if (
            ...
            # grow GSs
            n_dupli, n_split = self._grow_gs(params, optimizers, state, step, cap_max=cap_max)
            if self.verbose:
                print(
    ...
    def _grow_gs(
        ...
        # After modification operations, check and prune if above cap_max
        current_count = len(params["means"])
        if cap_max is not None:
            if current_count > cap_max:
                excess_count = current_count - cap_max
                prune_mask = torch.zeros(current_count, dtype=torch.bool, device=device)
                prune_mask[-excess_count:] = True  # Prune the most recently added GSs
                remove(params=params, optimizers=optimizers, state=state, mask=prune_mask)
                if self.verbose:
                    print(f"Pruned {excess_count} excess GSs to maintain the cap at {cap_max}")
